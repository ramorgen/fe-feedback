<template>
   <article
      ...
      justify-between
      items-center
      class="bg-#373F68 md:rounded-10px text-white"
   >
      <div flex items-center>
         <svg
            v-if="store.wWidth > 767"
            ml-24px
            width="23"
            height="24"
         ...
         <p v-if="store.wWidth > 767" h3 ml-16px>{{ suggestionsCount }} Suggestions</p>
         <!-- DropDown -->
         <div @click="showMenu = !showMenu" ml-24px md:ml-38px flex items-center cursor-pointer>
            <p text-14px class="text-#F2F4FE">Sort by :</p>
            <p class="text-#F2F4FE font-bold text-14px ml-5px">
               {{ activeOption }}
            </p>
            <div text-white ml-10px v-if="!showMenu">
               <chevronDown />
            </div>
            <div text-white ml-10px v-else>
               <chevronUp />
            </div>
         </div>
      </div>
      <div>
         <button @click="addClicked()" mr-16px btn btn1>+ Add Feedback</button>
      </div>
      <!-- Dropdown menu -->
      <DropDown
         w-255px
         ...
         left-40px
         md:left-220px
         :items="store.sortByData"
         :show="showMenu"
         :checkId="store.sortById"
...
<script setup>
   ...
   const suggestionsCount = computed(() => {
      return store.getData().length
   })

   const activeOption = computed(() => {
      return store.sortByData[store.sortByData.findIndex((item) => item.id === store.sortById)]
         .option
   })

   const showMenu = ref(false)

   const dropDownClicked = (id) => {
      store.sortById = id
   ...
   const addClicked = () => {
      ...
      store.lastComponent = store.activeComponent
